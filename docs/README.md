...
- **版本:v0.3.2——严重缺陷,影响 v0.2.0–v0.3.1 全部已发布版本**:在
  `10.36.8.10` 用真实浏览器走"新增设备"验证 v0.3.1 时发现新建的设备加不
  进去。排查发现 `deploy/nginx/Dockerfile` 构建前端时只设了
  `ENV VITE_API_BASE=`(留空,同源 `/api`,这本身没错),但漏了
  `ENV VITE_API_MODE=http`;`src/api/config.ts` 的判断是
  `VITE_API_MODE === 'http' || VITE_API_BASE`,BASE 留空时恒假,MODE 落到
  `mock`——Vite 构建期内联 `import.meta.env.*`,MODE 在编译期就是字面量
  `'mock'`,每个资源模块访问真实后端的 `fetch("/api/...")` 分支被当成死代码
  整个删掉。扒了已部署的前端 JS 包实测:里面只有 `/api/auth/login`,
  `/api/devices`、`/api/users`、`/api/policies`、`/api/sessions`、
  `/api/reports` 等一个字符串都没有——说明**控制台除登录页外全线只能看
  内置假数据,任何增删改都没有真正落到后端**,这个状态从 v0.2.0 首次发版
  起就存在,这次是第一次被发现,因为此前所有验证都是拿 `curl` 直接打后端
  API,没有通过真实浏览器把控制台走一遍。修复:补上
  `ENV VITE_API_MODE=http`(PR #18),本地验证过加/不加这行时构建产物里
