/**
 * 24 小时 / 近 7 天认证趋势图 — 逐点移植原型 dashboard.html 的 SVG 渲染逻辑
 * (坐标系、网格步长、轴标签完全一致),样式走 Tailwind 描边/填充工具类。
 */
const W = 720;
const H = 260;
...
export default function TrendChart({ series }: { series: TrendSeries }) {
  ...
  return (
    ...
    >
      {gridLines.map((t) => (
        <g key={t}>
          <line className="stroke-line-soft" strokeWidth={1} x1={P.l} y1={y(t)} x2={W - P.r} y2={y(t)} />
          <text className="fill-meta font-sans text-[10.5px]" x={P.l - 7} y={y(t) + 3.5} textAnchor="end">
            {t >= 1000 ? `${t / 1000}k` : t}
          </text>
        </g>
      ))}
      {ticks.map((i) => (
        <text key={i} className="fill-meta font-sans text-[10.5px]" x={x(i)} y={H - 7} textAnchor="middle">
          {label(i)}
        </text>
      ))}
      <path className="fill-accent opacity-[0.07]" d={areaPath} />
      <polyline className="fill-none stroke-accent" strokeWidth={2} points={okPts.join(' ')} />
      <polyline className="fill-none stroke-danger" strokeWidth={2} points={pts(fail).join(' ')} />
    </svg>
  );
}
